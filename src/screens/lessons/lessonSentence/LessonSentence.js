...
class lessonSentence extends Component {
  ...
  async componentDidMount() {
    try{
      this._sub1 = this.props.navigation.addListener('focus', () => {
        ...
        global.appState = AppState.addEventListener('change', this._handleAppStateChange);
  ...
  lessonVia = async () => {
    ...
    const learnLanguageString = await AsyncStorage.getItem('learning_language_id')
    const learnLanguageparsed = JSON.parse(learnLanguageString);
    // console.log("learnLanguageparsed", learnLanguageparsed);
    this.setState({learnLang:learnLanguageparsed});

    const data = {
      lesson_id: this.state.lessonDetails.lesson_id,
      baselesson_id: this.state.lessonDetails.baselesson_id,
      learning_language_id: learnLanguageparsed
    };
    ...
    doPost(`${apiUrls.sentencesByLessonId}`, data).then(async res => {
      ...
      if(res.success){
        ...
        // var ArrayPath = [];
        var alpha = store_path + '/cureentAudio' + `/${this.state.lessonDetails.lesson_id}`;
        ...
        console.log('break');
        RNFS.unlink(alpha);
        RNFS.mkdir(store_path + '/cureentAudio' + `/${this.state.lessonDetails.lesson_id}` );
        store_path = store_path + '/cureentAudio' + `/${this.state.lessonDetails.lesson_id}`;
        // var newArraySound = [];
        // var download_Array = [];
        // var is_downloaded = false;
        RNFS.readDir(store_path).then(async result => {
          console.log('result', result)
          for(let i = 0; i < res?.data?.sentenceslist.length; i++){
            if(res.data.sentenceslist[i].sound){
              let audioPath = store_path + '/' + i + 'audio.mp3';
              const soundPath = await this.downloadFile(res.data.sentenceslist[i].base64, audioPath);
              const sound = await this.loadSound(soundPath);
              // console.log("sound", sound);
              soundData.push({soundFile: sound, index:index})
              res.data.sentenceslist[i].soundFile = sound
              index++;
            }
            if(res.data.sentenceslist[i].type == "slow_song_with_sentence"){
              this.setState({speakIndex: i-1});
            }
            if (res.data.sentenceslist[i].type == 'sentenceList') {
              this.setState({sentenceListIndex: i});
            }
            if (res.data.sentenceslist[i].type == 'songLoop') {
              this.setState({listenPassiveIndex: i});
            }
            if (res.data.sentenceslist[i].state == 'repeat') {
              repeatDataArray.push(res.data.sentenceslist[i])
            }
          }
        }).catch(err => console.log("err", err))
        
        this.getTime(res.data.progress.time_loop);
        var SongDataArr = [];
        res.data.sentenceslist.map((songData, index) => {
        ...
        this.setState({
          lessions: res?.data.sentenceslist,
          ItemSong: SongDataArr,
          dataStates: res?.data.test,
          soundData: soundData,
            ...
            passive: {
              indexArray:
                res?.data?.progress?.passive_indexArray != 0
                  ...
                  : [],
              total: res.data.test.passive,
              max: res?.data?.progress?.passive_read,
            },
            speak: {
              indexArray:
                res?.data?.progress?.speak_indexArray != 0 ? res?.data?.progress.speak_indexArray : [],
              total: res.data.test.speak,
              max: res?.data?.progress.speak_read,
            },
            repeat: {
              indexArray:
                res?.data?.progress?.repeat_indexArray != 0
                  ? res?.data?.progress?.repeat_indexArray
                  : [],
              total: res.data.test.repeat,
  ...
  handleInternetConnectivity() {
    this.unsubscribe = NetInfo.addEventListener( async state => {
      ...
      if(!state.isConnected){
        ...
        if (this.state.lessonDetails.downloaded == 1) {
          this.downloadLesson(this.state.lessonDetails);
        }
      }else{
  ...
  playActive = async (item, index,) => {
    global.lessonActiveIndex = index;
    if(Platform.OS == "android"){
  ...
  pauseSoundLoop = async (index, item) => {
  ...
  saveLessonHistory = async (type) => {
    this.setState({multiRunning: false, isLoading: true});
    
    console.log('this.state.lessonDetails', this.state.lessonDetails);
    if (this.state.lessions.length >= 0) {
      const historyData = {
        lesson_id: this.state.lessonDetails.lesson_id,
        lessonfamily_id: this.state.lessonDetails.lessonfamily_id,
  ...
  downloadLesson = async (item) => {
    console.log('this.state.historyData from downloadLesson',this.state);
    let prog = 0;
    if (this.state.currentIndex >= this.state.lessonDetails.max_read_slide) {
      ...
      prog =
        (this.state.lessonDetails.max_read_slide * 100) /
        this.state.lessions.length;
    }
    ...
    // let passiveReadLength = this.state.historyData.passive.indexArray.length;
    ...
    RNFS.writeFile(store_path + '/info.json', JSON.stringify(info), 'utf8').then(success => {
      ...
      // type != 'dontGo' && this.state.getScreen == 'downloadList'
  ...
  async onTabClick(num, index) {
    ...
    this.setState({tabNum: num, toggle: false, isLoading: true,})
    AudioServiceModule.stopSound();
    if (num == 1) {
      if (this.state.lessions.length > 0) {
        this.input.onButtonStop();
  ...
  checkCurrentTab = async (type, index, state) => {
    try{
      ...
      if (array[global.lessonActiveIndex] && array[global.lessonActiveIndex].toggle) {
        array[global.lessonActiveIndex].toggle = 'pause';
      }
      if (array[lastRepeatIndex] && array[lastRepeatIndex].toggle) {
        array[lastRepeatIndex].toggle = 'pause';
      }
  ...
  _renderShowMessage = (item, index) => {
    ...
    return (
      <CustomInstruction
        ...
        goToLesson={index => this.saveLessonHistory(1)
                        ...
                        <View>
                          <TouchableOpacity
                            ...
                            onPress={() => this._getSelectSongIndexLoop( index, item )
                              ...
                              <View>
                                <Text style={styles.sentance_text_1} opacity={1}>
                                  {data?.sentence1}
                                </Text>
                              </View>
                          </TouchableOpacity>
                          {sentence3 != '' ? (
                            <TouchableOpacity
                              ...
                              <Text style={styles.loop_sub_sentance_text} opacity={1}>
                                {data?.sentence3}
                              </Text>
                            </TouchableOpacity>
                          ) : null}
                      ...
                      return (
                        <View>
                          <TouchableOpacity
                            ...
                            <Text style={styles.sentance_loop_text} opacity={1}>
                              {data?.sentence1}
                            </Text>
                          </TouchableOpacity>
                          {sentence3 != '' ? (
                            <TouchableOpacity
                              ...
                              <Text style={styles.loop_latin_sentance_text} opacity={1}>
                                {data?.sentence3}
                              </Text>
                            </TouchableOpacity>
                          ) : null}
