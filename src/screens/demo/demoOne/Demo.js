...
class DemoOne extends Component {
  ...
  async componentWillReceiveProps(newProps) { 
    var progress =
      newProps &&
      newProps.demoLesson &&
      newProps.demoLesson.demoLessonDetails &&
      newProps.demoLesson.demoLessonDetails.progress;

    var stateData =
      newProps &&
      newProps.demoLesson &&
      newProps.demoLesson.demoLessonDetails &&
      newProps.demoLesson.demoLessonDetails.stateCount;
      
    var data =
      newProps &&
      newProps.demoLesson &&
      newProps.demoLesson.demoLessonDetails.demo;
    
    var modifyDate =
      newProps &&
      newProps.demoLesson &&
      newProps.demoLesson.demoLessonDetails.lastModified; 

    this.secondsToTime(progress?.time_loop);
    ...
    this.setState({
      historyData: {
        ...
        repeat: {
          indexArray:
            progress?.repeat_indexArray != 0 ? progress?.repeat_indexArray : [],
    ...
    var ArrayPath = [];
    var alpha =
      store_path + '/cureentAudio' + `/${this.state.demoDetails.lesson_id}`;
    RNFS.stat(alpha)
      .then(stats => {
    ...
    await RNFS.readDir(store_path).then(async result => {
      if (result.length != 0) {
        is_downloaded = true;
        download_Array = result;
      }
      let count = 0;
      await data.reduce(async (promise, ele, i) => {
        ...
        if (ele.base64) {
          ...
          try {
            if (is_downloaded) {
              AsyncStorage.getItem('download_date', async (err1, item1) => {
                if (item1 != null) {
                  let lessonDate = JSON.parse(item1);
                  const storeDate =
                    lessonDate[this.state.demoDetails.lesson_id];

                  if (storeDate < modifyDate) {
                    audioPath = await this.downloadFile(ele.base64, audioPath);
                    var lessonDownloadDate =
                      (await AsyncStorage.getItem('download_date')) ||
                      JSON.stringify({});
                    lessonDownloadDate = JSON.parse(lessonDownloadDate);
                    lessonDownloadDate[
                      this.state.demoDetails.lesson_id
                    ] = modifyDate; 
                    AsyncStorage.setItem(
                      'download_date',
                      JSON.stringify(lessonDownloadDate),
                    ).then(async () => {});
                  } else {
                    audioPath = download_Array[count]?.path;
                  }
                }
              });
            } else {
              ...
              var lessonDownloadDate =
                ...
                JSON.stringify({}); 
              lessonDownloadDate = JSON.parse(lessonDownloadDate); 
              lessonDownloadDate[this.state.demoDetails.lesson_id] = modifyDate; 
              AsyncStorage.setItem(
                'download_date',
                JSON.stringify(lessonDownloadDate),
              ).then(async () => {});
            ...
            count++;
            try {
              ele.sound = await this.loadSound(audioPath);
            } catch {
              ele.sound = await this.loadSound(ele.sound);
            }

            ArrayPath.push(ele.sound);
          } catch {
            ele.sound = await this.loadSound(ele.sound);
            ArrayPath.push(ele.sound);
          }
        ...
        newArraySound.push(ele);
      }, Promise.resolve());
    });

    this.setState({
      lessions: newArraySound,
      lessonProgress: progress,
      dataStates: stateData,
      isApi: true,
    }, () => {
        if (newArraySound.length > 0) {
          this._getLastSoundList();
        }
      },
    ); 
    this.setState({passiveTimeloop : progress?.time_loop});
  }
        ...
        AsyncStorage.getItem('learning_language_id', (err2, item2) => {
          if (item2 != null) {
            ...
            const data = {
              ...
              // baselesson_id: this.props?.route?.params?.demoDetails?.baselesson_id,
            }; 
  ...
  _getLastSoundList = () => {
    let lastSoundArray = [];
    let sentanceSoundArray = [];
    let fastSentanceSoundArray = [];
    let repeatArray = [];
    Promise.all(
      this.state.lessions.map(async (ele, index) => {
        if (ele.type == 'sentenceList') {
          this.setState({sentenceListIndex: index});
        }
        if (ele.state == 'repeat') {
          repeatArray.push(index);
        }
        if (ele.type == 'songLoop') {
          this.setState({listenPassiveIndex: index});
          await Promise.all(
            ele.data.map(async (ele, index) => {
              await lastSoundArray.push({sound: ele.sound, index: index});
            }),
          );
        } else if (ele.type == 'slow_song_with_sentence') {
          this.setState({speakIndex: index-1});
          await Promise.all(
            ele.data.map(async (ele, index) => {
              if (ele.sound)
                await sentanceSoundArray.push({
                  sound: ele.sound,
                  index: index,
                  toggle: ele.toggle,
                });
        ...
        // else if (ele.type == 'fast_song_with_sentence') {
        //   await Promise.all(
        //     ele.data.map(async (ele, index) => {
        //       if (ele.sound) {
        //         await fastSentanceSoundArray.push({
        //           sound: ele.sound,
        //           index: index,
        //           toggle: ele.toggle,
        //         });
    ...
    this.setState({
      soundDataSentance: {
        slow: sentanceSoundArray,
        // fast: fastSentanceSoundArray,
      },
      repeatData: repeatArray,
    }); 
    this.setState({lastSoundData: lastSoundArray}, () => {
        ...
        <View
          ...
          <View style={styles.sound_btn_container}>
              ...
              <View>
                {item?.toggle != undefined && item?.toggle == 'pause' ? (
                  <TouchableOpacity
                    style={{}}
                    onPress={() => {
                      this.playActive(item, item.index)}
                    }
                    >
                ...
                {item?.toggle != undefined && item?.toggle == 'play' ? (
                  <TouchableOpacity
                    style={{}}
                    onPress={() => this.pauseActive(item.index)}>
                    <Image source={images.pauseIcon} />
                  </TouchableOpacity>
                ) : null}
                {item?.toggle != undefined && item?.toggle == 'refresh' && (
                  <TouchableOpacity
                    style={{}}
                    onPress={() => {
                      this.playActive(item, item.index)}
            ...
            <TouchableOpacity
              ...
              onPress={() => this.Langsam(i, item.sound)}>
            ...
            <ProgressCircle
              ...
              <View>
                {this.state.loopRunning == false ? (
                  ...
                  <TouchableOpacity
                    style={{}}
                    onPress={() => this.playPauseLoop(i, item.data)}>
                    <Image source={images.pauseIcon} />
                  </TouchableOpacity>
                ) }
              </View> 
  ...
  playActive = (item , index) => {
    ...
    if(Platform.OS == "android"){
      ...
      AudioServiceModule.playActiveSound(item.sound._filename, this.state.active_Text);
  ...
  _getSelectSongIndexLoop = (index, item, selectIndex) => {
    this.sentanceLoopIndex = selectIndex;
    AudioServiceModule.stopSound();
  ...
  sentance_loop1 = async (index, item) => {
    try{
      if(Platform.OS == "android"){
        ...
        this.setState({multiRunning: true});
        goBackIndex = index;
        const {speedType} = this.state;

        let SoundArray = this.state.soundDataSentance[speedType].filter(ele => {
          if (ele.index >= this.sentanceLoopIndex) {
            return ele;
          }
        });
        ...
        var indexArray = [];
        for(let i = 0; i < SoundArray.length; i++ ){
          track.push(SoundArray[i].sound._filename,)
          indexArray.push(SoundArray[i].index);
        }
        console.log('track', track);
  ...
  playRepeatSound = async (item, index) => {
    if(Platform.OS == "android"){
      ...
      this.setState({lessions: array, songType: 'solo'});
      AudioServiceModule.playRepeat(item.sound._filename, this.state.active_Text).then(success => {
      ...
      this.setState({lessions: array, songType: 'solo'});
      AudioServiceModule.playRepeat(item.sound._filename, this.state.active_Text);
      const eventEmitter = new NativeEventEmitter(AudioServiceModule);
  ...
  playSoundLoop = async (index, item,) => {
    if(Platform.OS == "android"){
      ...
      let SoundArray = this.state.lastSoundData.filter(ele => {
        return ele;
      });
      console.log("SoundArray", SoundArray);
  ...
  _renderSlowSongSentanceStep = (item, i, type) => {
    goBackIndex = i;
    const {speedType} = this.state;
    if (this.state.soundDataSentance[speedType]) {
      let songIndex =
        this.state.soundDataSentance[speedType][this.sentanceLoopIndex] &&
        this.state.soundDataSentance[speedType][this.sentanceLoopIndex].index;
                ...
                <View>
                  {this.state.multiRunning == false ? (
                    <TouchableOpacity
                      style={{}}
                      onPress={() => {
                        this.sentance_loop1(i, item.data)
                        console.log("jjd");
                      }}>
                      <Image source={images.playIcon} style={styles.playIcon} />
                    </TouchableOpacity>
  ...
  goToSubscription = () => {
    this.setState({navType: 'subscrib'});
    ...
    return (
      <View style={{flex: 1}}>
        ...
        <View
          ...
          <BlueButton
            title={this.state.appString.see_subscription}
  ...
  checkCurrentTab = async (type, index, state) => {
    ...
    let array1 = this.state.lessions;
    
    if (array1[global.activeIndex] && array1[global.activeIndex].toggle) {
      array1[global.activeIndex].toggle = 'pause';
    }
    if (array1[lastRepeatIndex] && array1[lastRepeatIndex].toggle) {
      array1[lastRepeatIndex].toggle = 'pause';
    }
    this.setState({lessions: array1});

    console.log('tabState',this.state.tabState, this.state.activeRunning)
    if (state != 'repeat') {
      this.historyAdd(state, index);
    }
    this.setState({activeState: state});

    // if (this.state.activeState != state) {
      // if (this.state.getScreen != 'downloadList') {
        // this.state.isFirst != true ? this.input.onButtonBackground() : '';
      // } else {
        // this.input.onButtonStop('dontGo');
      // }
    // }
  ...
  render() { 
    ...
    console.log("this.state.passiveTimeloop", this.state.passiveTimeloop);
    return (
